<?php
/**
 * AV-Vertrag Installation - Standalone Version
 * Keine Abhängigkeiten, keine Includes
 */

// WICHTIG: Keine Ausgabe vor diesem Punkt!
error_reporting(0); // Unterdrücke alle PHP-Fehler für sauberes JSON
ini_set('display_errors', 0);

// POST-Request = Installation durchführen
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'install') {
    
    // Buffer alles bis wir bereit sind
    while (ob_get_level()) {
        ob_end_clean();
    }
    ob_start();
    ...
    try {
        // Datenbank-Credentials (direkt hier, keine Includes)
        $host = 'localhost';
        $database = 'lumisaas';
        $username = 'lumisaas52';
        ...
        try {
            ...
            $checks[] = ['status' => 'ok', 'message' => 'Datenbankverbindung hergestellt'];
        } catch (PDOException $e) {
            $checks[] = ['status' => 'error', 'message' => 'DB-Verbindung fehlgeschlagen: ' . $e->getMessage()];
        ...
        // Schritt 2: Prüfe ob users Tabelle existiert
        try {
            $stmt = $pdo->query("SHOW TABLES LIKE 'users'");
            if ($stmt->rowCount() > 0) {
                $checks[] = ['status' => 'ok', 'message' => 'Users-Tabelle vorhanden'];
            } else {
                $checks[] = ['status' => 'error', 'message' => 'Users-Tabelle fehlt'];
            ...
            echo json_encode([
                'success' => false,
                'message' => 'System-Checks fehlgeschlagen',
                'error' => $e->getMessage(),
                'checks' => $checks
            ], JSON_UNESCAPED_UNICODE);
            ob_end_flush();
        ...
        // Schritt 3: Prüfe ob Tabelle bereits existiert
        $stmt = $pdo->query("SHOW TABLES LIKE 'user_company_data'");
        $tableExists = $stmt->rowCount() > 0;
        $checks[] = [
            'status' => 'ok', 
            'message' => $tableExists ? 'Tabelle existiert (wird aktualisiert)' : 'Tabelle wird erstellt'
        ];
        
        // Schritt 4: Erstelle Tabelle
        $sql = "CREATE TABLE IF NOT EXISTS user_company_data (
...
<body>
    <div class="container">
        ...
        <div class="warning">
            ...
            Die Installation ist sicher und kann mehrfach ausgeführt werden.
        </div>
        ...
        <div class="step">
            ...
            <div class="step-content">
                <ul>
                    ...
                    <li>Foreign Key zu users-Tabelle</li>
                    <li>Alle notwendigen Indizes</li>
                </ul>
            </div>
        </div>
    ...
    <script>
        async function startInstallation() {
            ...
            try {
                ...
                // Hole den Text der Response
                const text = await response.text();
                
                // Debug-Ausgabe
                debugDiv.textContent = 'Server Response:\n' + text.substring(0, 500);
                ...
                try {
                    ...
                    throw new Error('Server hat kein gültiges JSON zurückgegeben. Siehe Debug-Box unten.');
                }
                ...
                if (data.success) {
                    ...
                    result.innerHTML = `
                        ...
                        1. Gehe zu: Dashboard → Einstellungen<br>
                        2. Fülle das Firmendaten-Formular aus<br>
                        3. Lade deinen AV-Vertrag herunter<br><br>
                        <strong>⚠️ Wichtig:</strong> Lösche diese Datei nach der Installation!
                    `;
                    installBtn.textContent = '✅ Installation abgeschlossen';
                    debugDiv.style.display = 'none'; // Verstecke Debug bei Erfolg
                } else {
                    result.className = 'result error';
                    result.innerHTML = `
                        <strong>❌ Installation fehlgeschlagen</strong><br><br>
                        ${data.message || 'Unbekannter Fehler'}<br><br>
                        ${data.error ? `<strong>Details:</strong><br>${data.error}` : ''}
                    `;
                ...
                result.innerHTML = `
                    ...
                    ${error.message}<br><br>
                    Bitte prüfe die Debug-Box unten und die Browser-Konsole (F12).
                `;
                installBtn.disabled = false;
                installBtn.textContent = '🔄 Erneut versuchen';
            }
