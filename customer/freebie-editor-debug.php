...
// Debug Mode aktivieren wenn ?debug=1 im URL
...
try {
    ...
    if ($debug_mode) {
        echo "<h2>DEBUG: Template-Daten aus DB</h2>";
        echo "<pre>";
        echo "headline: '" . ($template['headline'] ?? 'NULL') . "'\n";
        echo "subheadline: '" . ($template['subheadline'] ?? 'NULL') . "'\n";
        echo "preheadline: '" . ($template['preheadline'] ?? 'NULL') . "'\n";
        echo "bullet_points: '" . ($template['bullet_points'] ?? 'NULL') . "'\n";
        echo "cta_text: '" . ($template['cta_text'] ?? 'NULL') . "'\n";
        echo "</pre>";
    }
    ...
    // DEBUG: Customer-Freebie-Daten ausgeben
    if ($debug_mode) {
        echo "<h2>DEBUG: Customer-Freebie-Daten</h2>";
        if ($customer_freebie) {
            echo "<pre>";
            echo "headline: '" . ($customer_freebie['headline'] ?? 'NULL') . "'\n";
            echo "subheadline: '" . ($customer_freebie['subheadline'] ?? 'NULL') . "'\n";
            echo "preheadline: '" . ($customer_freebie['preheadline'] ?? 'NULL') . "'\n";
            echo "bullet_points: '" . ($customer_freebie['bullet_points'] ?? 'NULL') . "'\n";
            echo "cta_text: '" . ($customer_freebie['cta_text'] ?? 'NULL') . "'\n";
            echo "</pre>";
        } else {
            echo "<p>Kein Customer-Freebie gefunden (Template wurde noch nicht verwendet)</p>";
        }
    }
...
// Form data zusammenstellen - DIREKT aus Template wenn kein customer_freebie existiert
$form_data = [
...
// DEBUG: Endgültige Form-Daten ausgeben
if ($debug_mode) {
    echo "<h2>DEBUG: Finale Form-Daten</h2>";
    echo "<pre>";
    print_r($form_data);
    echo "</pre>";
    echo "<hr>";
