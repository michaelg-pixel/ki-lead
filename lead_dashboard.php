...
// Umleitung auf Registrierungsseite wenn nicht eingeloggt
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .progress-bar-container { width: 100%; height: 8px; background: #e9ecef; border-radius: 10px; overflow: hidden; margin: 15px 0; }
        .progress-bar { height: 100%; background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); border-radius: 10px; transition: width 0.3s ease; }
        .progress-indicator { font-size: 14px; font-weight: 600; color: #667eea; }
        ...
        .reward-card { background: white; border: 2px solid #e9ecef; border-radius: 12px; padding: 20px; margin-bottom: 15px; }
        .reward-header { display: flex; justify-content: space-between; align-items: start; margin-bottom: 10px; }
        .reward-name { font-size: 18px; font-weight: 700; color: #333; }
        .reward-date { font-size: 12px; color: #999; }
        .reward-description { color: #666; line-height: 1.6; margin-bottom: 15px; }
        .reward-value { display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 8px 16px; border-radius: 20px; font-weight: 600; font-size: 14px; }
        ...
        <div class="content">
                    ...
                    <?php else: ?>
                        <div class="freebie-grid">
                            <?php foreach ($freebies as $freebie): 
                                $progress_percent = $freebie['total_lessons'] > 0 ? round(($freebie['user_progress'] / $freebie['total_lessons']) * 100) : 0;
                            ?>
            ...
            <div class="page-content <?php echo $current_page === 'kurse' ? 'active' : ''; ?>">
                ...
                if ($freebie_id) {
                    ...
                    echo '<p style="color: #666; margin-top: 10px; margin-bottom: 30px;">Hier findest du alle deine verfügbaren Kurse und Trainings.</p>';
                    
                    if (empty($freebies)) {
                        echo '<div class="empty-state"><i class="fas fa-graduation-cap"></i><h3>Noch keine Kurse verfügbar</h3><p>Neue Kurse werden hier erscheinen, sobald sie verfügbar sind.</p></div>';
                    } else {
                        echo '<div class="freebie-grid">';
                        foreach ($freebies as $freebie) {
                            $progress_percent = $freebie['total_lessons'] > 0 ? round(($freebie['user_progress'] / $freebie['total_lessons']) * 100) : 0;
                            echo '<div class="freebie-card">';
                            if ($freebie['mockup_image']) {
                                echo '<img src="' . htmlspecialchars($freebie['mockup_image']) . '" alt="' . htmlspecialchars($freebie['name']) . '" class="freebie-mockup">';
                            } else {
                                echo '<div class="freebie-mockup"></div>';
                            }
                            echo '<div class="freebie-content">';
                            echo '<div class="freebie-title">' . htmlspecialchars($freebie['name']) . '</div>';
                            echo '<div class="freebie-description">' . htmlspecialchars($freebie['description']) . '</div>';
                            
                            if ($progress_percent > 0) {
                                echo '<div class="progress-bar-container"><div class="progress-bar" style="width: ' . $progress_percent . '%"></div></div>';
                                echo '<div class="progress-indicator">' . $progress_percent . '% abgeschlossen</div>';
                            }
                            
                            echo '<div class="freebie-meta"><span class="lesson-count"><i class="fas fa-play-circle"></i>' . $freebie['total_lessons'] . ' Lektionen</span></div>';
                            echo '<button class="start-btn" onclick="window.location.href=\'?page=kurse&freebie=' . $freebie['id'] . '\'">' . ($progress_percent > 0 ? 'Fortsetzen' : 'Starten') . '</button>';
                            echo '</div></div>';
                        }
                        echo '</div>';
                    }
