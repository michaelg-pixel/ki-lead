<?php
// Emergency Recovery für marktplatz.php (Meine Marktplatz-Angebote)
echo "<h2>🚨 Emergency Recovery - Meine Marktplatz-Angebote</h2>";

// WICHTIG: Diese Datei muss nach /customer/sections/marktplatz.php
$target_file = __DIR__ . '/customer/sections/marktplatz.php';

$marktplatz_content = <<<'PHP'
<?php
...
if (!is_dir($dir)) {
    mkdir($dir, 0755, true);
    echo "✅ Verzeichnis erstellt: $dir<br>";
}

// Schreibe Datei
if (file_put_contents($target_file, $marktplatz_content)) {
    echo "✅ marktplatz.php wiederhergestellt nach /customer/sections/<br>";
    echo "✅ Größe: " . strlen($marktplatz_content) . " Bytes<br>";
    
    // Syntax Check
    exec("php -l " . escapeshellarg($target_file) . " 2>&1", $output, $return_code);
    
    if ($return_code === 0) {
        echo "✅ Syntax Check: OK<br>";
    } else {
        echo "❌ Syntax Fehler:<br><pre>" . implode("\n", $output) . "</pre>";
    }
    
} else {
    echo "❌ Fehler beim Schreiben der Datei<br>";
}

echo "<br><strong>Beide Marktplatz-Seiten sollten jetzt funktionieren!</strong><br>";
echo "<a href='/customer/dashboard.php?page=marktplatz' style='display: inline-block; margin: 10px; padding: 12px 24px; background: #667eea; color: white; text-decoration: none; border-radius: 8px; font-weight: 600;'>🏪 Meine Marktplatz-Angebote testen</a>";
echo "<a href='/customer/dashboard.php?page=marktplatz-browse' style='display: inline-block; margin: 10px; padding: 12px 24px; background: #764ba2; color: white; text-decoration: none; border-radius: 8px; font-weight: 600;'>🛍️ Marktplatz durchsuchen testen</a>";
?>
